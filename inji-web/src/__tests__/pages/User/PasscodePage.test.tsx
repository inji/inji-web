...
describe('Passcode', () => {
    ...
    test("should show Log in with different account button when wallet is temporarily locked", async () => {
        mockApiResponseSequence([{
            data: [{
                walletId: "2c2e1810-19c8-4c85-910d-aa1622412413",
                walletName: null,
        ...
        await waitFor(() => {
            expect(screen.getByTestId("error-msg-passcode-temporarily-locked")).toBeInTheDocument();
        });

        expect(screen.getByTestId("btn-login-different-account")).toBeInTheDocument();
        expect(screen.getByTestId("btn-login-different-account")).toHaveTextContent("Log in with different account");
    });

    test("should not show Log in with different account button when wallet is permanently locked", async () => {
        mockApiResponseSequence([{
            data: [{
                walletId: "2c2e1810-19c8-4c85-910d-aa1622412413",
                walletName: null,
        ...
        await waitFor(() => {
            expect(screen.getByTestId("error-msg-passcode-permanently-locked")).toBeInTheDocument();
        });

        expect(screen.queryByTestId("btn-login-different-account")).not.toBeInTheDocument();
    });

    test("should logout and redirect to root when Log in with different account is clicked in temporarily locked state", async () => {
        ...
        userEvent.click(screen.getByTestId("btn-login-different-account"));
