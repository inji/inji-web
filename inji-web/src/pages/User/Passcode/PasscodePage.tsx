...
const MAX_FAILED_ATTEMPTS = 5; // Default max attempts, matching wallet.passcode.maxFailedAttemptsAllowedPerCycle config
    ...
    const handleWalletStatusError = (errorCode: string, fallBackError: string | undefined = undefined, httpStatusCode: number | null = null) => {
        if (
            ...
            if (errorCode !== WalletLockStatus.LAST_ATTEMPT_BEFORE_LOCKOUT) {
                ...
                // Reset failed attempts when wallet is locked (temporarily or permanently)
                setFailedAttempts(0);
            }
            // For last_attempt_before_lockout, don't reset attempts as backend is tracking
        } else if (fallBackError) {
            if (httpStatusCode === HTTP_STATUS_CODES.BAD_REQUEST) {
                // Increment failed attempts for incorrect passcode
                const newFailedAttempts = failedAttempts + 1;
                setFailedAttempts(newFailedAttempts);
                const remainingAttempts = MAX_FAILED_ATTEMPTS - newFailedAttempts;

                if (remainingAttempts > 0) {
                    setError(t('error.incorrectPasscodeWithAttempts', {
                        attempt: newFailedAttempts,
                        maxAttempts: MAX_FAILED_ATTEMPTS,
                        remainingAttempts: remainingAttempts
                    }));
                } else {
                    // This shouldn't happen as backend should lock before this, but handle it anyway
    ...
    const fetchWallets = async () => {
        try {
            ...
            if (wallets && wallets.length > 0) {
                ...
                // Reset failed attempts when wallet status changes (e.g., after lock expires)
                if (walletStatus !== WalletLockStatus.TEMPORARILY_LOCKED &&
                    walletStatus !== WalletLockStatus.PERMANENTLY_LOCKED &&
                    walletStatus !== WalletLockStatus.LAST_ATTEMPT_BEFORE_LOCKOUT) {
                    setFailedAttempts(0);
                }
